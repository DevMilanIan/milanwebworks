...
export function SocialMedia({ className, invert = false }: { className?: string; invert?: boolean }) {
  return (
    <ul role="list" className={clsx('flex gap-x-10', invert ? 'text-brand-400' : 'text-brand-50', className)}>
      {socialMediaProfiles.map((socialMediaProfile) => (
        <li key={socialMediaProfile.title}>
          <Link
            href={socialMediaProfile.href}
            aria-label={socialMediaProfile.title}
            className={clsx('transition', invert ? 'hover:text-brand-600' : 'hover:text-brand-200')}
          >
